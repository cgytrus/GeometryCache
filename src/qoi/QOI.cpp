...
// Modifications this code are licensed under the standard MIT license.
// Copyright (C) 2022 ConfiG
...
bool QOIEncoder::encode(int width, int height, uint8_t const * pixels, bool alpha, bool linearColorspace)
{
	...
	for (int pixelsOffset = 0; pixelsOffset < pixelsSize;) {
		int pixel = 0;
		if(alpha) {
			pixel |= pixels[pixelsOffset * 4] << 16;
			pixel |= pixels[pixelsOffset * 4 + 1] << 8;
			pixel |= pixels[pixelsOffset * 4 + 2];
			pixel |= pixels[pixelsOffset++ * 4 + 3] << 24;
		}
		else {
			pixel |= pixels[pixelsOffset * 3] << 16;
			pixel |= pixels[pixelsOffset * 3 + 1] << 8;
			pixel |= pixels[pixelsOffset++ * 3 + 2];
			pixel |= -16777216;
		}
		if (pixel == lastPixel) {
...
bool QOIDecoder::decode(uint8_t const * encoded, int encodedSize)
{
	...
	int pixelsSize = width * height;
	uint8_t* pixels = new uint8_t[pixelsSize * (this->alpha ? 4 : 3)];
	//std::shared_ptr<uint8_t[]> pixels = std::make_shared<uint8_t[]>(pixelsSize * (this->alpha ? 4 : 3));
	encodedSize -= 8;
	int encodedOffset = 14;
	std::array<int, 64> index {};
	int pixel = -16777216;
	for (int pixelsOffset = 0; pixelsOffset < pixelsSize;) {
		...
		case 0:
			pixel = index[e];
			if(this->alpha) {
				pixels[pixelsOffset * 4] = pixel >> 16 & 255;
				pixels[pixelsOffset * 4 + 1] = pixel >> 8 & 255;
				pixels[pixelsOffset * 4 + 2] = pixel & 255;
				pixels[pixelsOffset++ * 4 + 3] = pixel >> 24 & 255;
			}
			else {
				pixels[pixelsOffset * 3] = pixel >> 16 & 255;
				pixels[pixelsOffset * 3 + 1] = pixel >> 8 & 255;
				pixels[pixelsOffset++ * 3 + 2] = pixel & 255;
			}
			continue;
		case 1:
		...
		default:
			if (e < 254) {
				...
				if (pixelsOffset + e > pixelsSize)
					return false;
				if(this->alpha) {
					auto start = (uint8_t*)((uintptr_t)pixels + pixelsOffset * 4);
					for(int i = 0; i < e; ++i) {
						start[i * 4] = pixel >> 16 & 255;
						start[i * 4 + 1] = pixel >> 8 & 255;
						start[i * 4 + 2] = pixel & 255;
						start[i * 4 + 3] = pixel >> 24 & 255;
					}
				}
				else {
					auto start = (uint8_t*)((uintptr_t)pixels + pixelsOffset * 3);
					for(int i = 0; i < e; ++i) {
						start[i * 3] = pixel >> 16 & 255;
						start[i * 3 + 1] = pixel >> 8 & 255;
						start[i * 3 + 2] = pixel & 255;
					}
				}
				pixelsOffset += e;
				continue;
...
uint8_t const * QOIDecoder::getPixels() const
{
	return this->pixels;
}
